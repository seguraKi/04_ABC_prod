...
spark.read.parquet("/mnt/sppo-refined-dev/material_regional_dim").createOrReplaceGlobalTempView(TablePrefix+"mara_sl")
spark.read.parquet("/mnt/refined/material_dim").createOrReplaceGlobalTempView(TablePrefix+"material_dim")
spark.read.parquet("/mnt/refined/plant_dim").createOrReplaceGlobalTempView(TablePrefix+"plant_dim")
spark.read.parquet("/mnt/refined/geo_dim").createOrReplaceGlobalTempView(TablePrefix+"geo_dim")

SBM_Tables_v2.createTable('vw_sbm_material_dim', view_type)

abc_kinaxis_input_fact = spark.read.parquet("/mnt/sppo-refined-dev/abc_kinaxis_input_fact/")
planning_parameter_fact = spark.read.parquet("/mnt/refined/planning_parameter_fact")

planning_parameter_fact.createOrReplaceGlobalTempView(TablePrefix+"planning_parameter_fact")
abc_kinaxis_input_fact.createOrReplaceGlobalTempView(TablePrefix+"abc_kinaxis_input_fact")

sbm_material_dim = spark.sql("select * from global_temp."+TablePrefix+"vw_sbm_material_dim")
...
sql = """
  ...
  with shelf_life_temp as (
    ...
    select distinct
      ...
      , ipa.min_prod_quantity as min_prod_qty
      , ipa.production_family as abc_mat_fam_name
    ...
    from global_temp."""+TablePrefix+"""abc_kinaxis_input_fact kif
    inner join global_temp."""+TablePrefix+"""vw_sbm_material_dim md
      on cast(kif.material_id as int) = cast(md.material_number as int)
    inner join (
      select distinct
        plant_code
        , plant_name
        , gd.region_override 
      from global_temp."""+TablePrefix+"""plant_dim pd
      left join (
        SELECT distinct
          country_code,	region_name,	country_name,
          (CASE
              ...
              ELSE 'missing'
          END) as region_override
        from global_temp."""+TablePrefix+"""geo_dim
      ) gd 
        on pd.country_code = gd.country_code 
    ) pd  
      on pd.plant_code = kif.plant_code
    join shelf_life sl
      on kif.source_system_code = sl.source_system_code
      and kif.material_id = sl.material_id
      
    left join ivy_prod_agreement_"""+ rtEnvironment.lower()+""" ipa
      on ipa.vendor_id = kif.purchase_vendor_id
      and ipa.material_id = cast(kif.material_id as int)
