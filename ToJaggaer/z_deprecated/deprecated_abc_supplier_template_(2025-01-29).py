...
df_active_agr_fact = spark.read.parquet("/mnt/mda-prod/active_agreement_fact")
df_active_agr_fact.createOrReplaceGlobalTempView("active_agreement_fact")
...
from
  ...
  left join global_temp.active_agreement_fact aa on aa.material_id = kif.material_id AND aa.plant_code = kif.plant_code AND aa.purchase_vendor_id = kif.purchase_vendor_id
  ...
  AND dim.vendor_id = kif.purchase_vendor_id
  inner join (
    select
      distinct plant_code,
      plant_name,
...
abc_supplier_template = abc_supplier_template.dropDuplicates(["vendor_id", "material_id", "plant_code"])
