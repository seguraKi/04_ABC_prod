...
try:
  ...
  sql = """
      ...
      where 
        substring(requirement_date_for_the_component, 0,6) >= concat(cast(DATE_FORMAT(cast(getdate() as date), "yyyy") as int), DATE_FORMAT(cast(getdate() as date), "MM"))
        and substring(requirement_date_for_the_component, 0,6) <= concat(cast(DATE_FORMAT(cast(getdate() as date), "yyyy") + 1 as int), DATE_FORMAT(cast(getdate() as date), "MM"))
