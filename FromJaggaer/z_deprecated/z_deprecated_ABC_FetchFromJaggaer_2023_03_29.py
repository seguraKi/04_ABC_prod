...
abc_quotation_fact = abc_quotation_fact.withColumn("qty_forecast_n12",F.col("qty_forecast_n12").cast("decimal(12,2)"))\
  .withColumn("qty_forecast_n12_family",F.col("qty_forecast_n12_family").cast("decimal(12,2)"))\
  .withColumn("freight_factor",F.col("freight_factor").cast("decimal(10,2)"))\
  .withColumn("price_unit_factor",F.col("price_unit_factor").cast("int"))\
  .withColumn("min_prod_qty_sc1", F.col("min_prod_qty_sc1").cast("decimal(10,2)"))\
  .withColumn("runs_annual_sc1", F.col("runs_annual_sc1").cast("decimal(10,2)"))\
  .withColumn("net_price_sc1", F.col("net_price_sc1").cast("decimal(10,2)"))\
  .withColumn("min_prod_qty_sc2", F.col("min_prod_qty_sc2").cast("decimal(10,2)"))\
  .withColumn("runs_annual_sc2", F.col("runs_annual_sc2").cast("decimal(10,2)"))\
  .withColumn("net_price_sc2", F.col("net_price_sc2").cast("decimal(10,2)"))\
  .withColumn("min_prod_qty_sc3", F.col("min_prod_qty_sc3").cast("decimal(10,2)"))\
  .withColumn("runs_annual_sc3", F.col("runs_annual_sc3").cast("decimal(10,2)"))\
  .withColumn("net_price_sc3", F.col("net_price_sc3").cast("decimal(10,2)"))\
  .withColumn("min_prod_qty_sc4", F.col("min_prod_qty_sc4").cast("decimal(10,2)"))\
  .withColumn("runs_annual_sc4", F.col("runs_annual_sc4").cast("decimal(10,2)"))\
  .withColumn("net_price_sc4", F.col("net_price_sc4").cast("decimal(10,2)"))\
  .withColumn("min_prod_qty_sc5", F.col("min_prod_qty_sc5").cast("decimal(10,2)"))\
  .withColumn("runs_annual_sc5", F.col("runs_annual_sc5").cast("decimal(10,2)"))\
  .withColumn("net_price_sc5", F.col("net_price_sc5").cast("decimal(10,2)"))\
  .withColumn("min_prod_qty_sc6", F.col("min_prod_qty_sc6").cast("decimal(10,2)"))\
  .withColumn("runs_annual_sc6", F.col("runs_annual_sc6").cast("decimal(10,2)"))\
  .withColumn("net_price_sc6", F.col("net_price_sc6").cast("decimal(10,2)"))\
  .withColumn("min_prod_qty_sc7", F.col("min_prod_qty_sc7").cast("decimal(10,2)"))\
  .withColumn("runs_annual_sc7", F.col("runs_annual_sc7").cast("decimal(10,2)"))\
  .withColumn("net_price_sc7", F.col("net_price_sc7").cast("decimal(10,2)"))\
  .withColumn("min_prod_qty_sc8", F.col("min_prod_qty_sc8").cast("decimal(10,2)"))\
  .withColumn("runs_annual_sc8", F.col("runs_annual_sc8").cast("decimal(10,2)"))\
  .withColumn("net_price_sc8", F.col("net_price_sc8").cast("decimal(10,2)"))\
...
(abc_quotation_fact.write
  ...
  .option("qty_forecast_n12", "qty_forecast_n12 decimal(12,2)")
  .option("qty_forecast_n12_family", "qty_forecast_n12_family decimal(12,2)")
  .option("freight_factor", "freight_factor decimal(10,2)")
  .option("price_unit_factor", "price_unit_factor int")
  .option("min_prod_qty_sc1", "min_prod_qty_sc1 decimal(10,2)")
  .option("runs_annual_sc1", "runs_annual_sc1 decimal(10,2)")
  .option("net_price_sc1", "net_price_sc1 decimal(10,2)")
  .option("min_prod_qty_sc2", "min_prod_qty_sc2 decimal(10,2)")
  .option("runs_annual_sc2", "runs_annual_sc2 decimal(10,2)")
  .option("net_price_sc2", "net_price_sc2 decimal(10,2)")
  .option("min_prod_qty_sc3", "min_prod_qty_sc3 decimal(10,2)")
  .option("runs_annual_sc3", "runs_annual_sc3 decimal(10,2)")
  .option("net_price_sc3", "net_price_sc3 decimal(10,2)")
  .option("min_prod_qty_sc4", "min_prod_qty_sc4 decimal(10,2)")
  .option("runs_annual_sc4", "runs_annual_sc4 decimal(10,2)")
  .option("net_price_sc4", "net_price_sc4 decimal(10,2)")
  .option("min_prod_qty_sc5", "min_prod_qty_sc5 decimal(10,2)")
  .option("runs_annual_sc5", "runs_annual_sc5 decimal(10,2)")
  .option("net_price_sc5", "net_price_sc5 decimal(10,2)")
  .option("min_prod_qty_sc6", "min_prod_qty_sc6 decimal(10,2)")
  .option("runs_annual_sc6", "runs_annual_sc6 decimal(10,2)")
  .option("net_price_sc6", "net_price_sc6 decimal(10,2)")
  .option("min_prod_qty_sc7", "min_prod_qty_sc7 decimal(10,2)")
  .option("runs_annual_sc7", "runs_annual_sc7 decimal(10,2)")
  .option("net_price_sc7", "net_price_sc7 decimal(10,2)")
  .option("min_prod_qty_sc8", "min_prod_qty_sc8 decimal(10,2)")
  .option("runs_annual_sc8", "runs_annual_sc8 decimal(10,2)")
  .option("net_price_sc8", "net_price_sc8 decimal(10,2)") 
  .option("useAzureMSI", "true")
  .option("tempDir","abfss://temp@saproddatahub.dfs.core.windows.net/sbm")
  .option("user",jdbcUsername)
  .option("password",jdbcPassword)
...
abc_supplier_input_inprocess_dim = abc_supplier_input_inprocess_dim.withColumn("min_prod_qty",F.col("min_prod_qty").cast("decimal(10,2)"))
