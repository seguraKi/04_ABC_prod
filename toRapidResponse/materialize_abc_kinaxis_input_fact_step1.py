...
from pyspark.sql.functions import lower, regexp_replace, lpad, lit, coalesce,concat_ws,collect_list,collect_set,when, col,sum, count, round, countDistinct, length
...
df_material_dim = spark.read.parquet('/mnt/mda-prod/material_dim').alias('mat')
...
abc_filtered = abc.select('vendor_id', 'material_id','base_uom', 'abc_mat_fam_name', 'abc_mat_fam_new_msm', 'restriction_min_prod_qty', 'restriction_max_prod_qty','plant_code','supplier_storage_flag', 'material_per_pallet_qty', 'min_prod_qty','ticket_id', 'setup_cost_usd', 'prod_rounding_value', 'material_shelf_life_months', 'vendor_plant_change_freq','supplier_input_repeats').distinct().alias('abc')
...
abc_df = (
    ...
    .join(
        df_material_dim,
        (F.col('sy.material_id') == F.col('mat.material_number')),
        'left'
    )
    .selectExpr(
        'sy.purchase_vendor_id',
        'sy.material_id',
        'sy.plant_code',
        #-
        'mat.material_desc',
        'abc.base_uom', 
        'abc.abc_mat_fam_name', 
        'abc.abc_mat_fam_new_msm', 
        'abc.restriction_min_prod_qty', 
        'abc.restriction_max_prod_qty',
        'abc.supplier_storage_flag', 
        'abc.material_per_pallet_qty', 
        'abc.min_prod_qty',
        'abc.ticket_id', 
        'abc.setup_cost_usd', 
        'abc.prod_rounding_value', 
        'abc.material_shelf_life_months', 
        'abc.vendor_plant_change_freq',
        'abc.supplier_input_repeats',
        #-
        ...
        "CASE WHEN tg.forecast_override_flag == 'yes' AND abc_fcst.qty_forecast_override IS NOT NULL THEN abc_fcst.qty_forecast_override ELSE sy.total_requirement_qty END AS qty_forecast_n12",
...
iopt_po_pi = (
    ivy_pallet_df.join(
        ...
        (F.col('comma1.iopt_initiative_pi_material_id') == F.col('ivy_pallet2.material_id')) & 
...
column_selected = column_selected.groupBy( 's.source_system_code','s.purchase_vendor_id', 's.plant_code', 's.material_id','s.material_desc','s.base_uom','future_flag_supplier_inv_storage', 's.price_unit_factor', 's.doc_curr_code', 's.net_price_rate', 's.price_purch_usd', 's.mm_minimum_lot_size', 's.mm_minimum_lot_size_dfc', 's.avg_batch_consumption_qty','s.qty_forecast_n12' ,'s.abc_mat_fam_name', 's.abc_mat_fam_new_msm', 's.restriction_min_prod_qty', 's.restriction_max_prod_qty', 's.supplier_storage_flag', 's.material_per_pallet_qty', 's.min_prod_qty', 's.min_prod_qty_dfc', 's.spend_pool_lkp_code', 's.spend_pool_group', 's.spend_pool_medium_name', 's.spend_pool_low_name', 's.business_unit_lkp_code', 's.sector_business_unit', 's.business_unit', 's.pipo_indicator', 'ticket_id', 'modified_date', 'ticket_owner', 'ticket_custom_id', 'ticket_title','pipo_horizon_days', 'setup_cost_usd', 'prod_rounding_value', 'material_shelf_life_months', 'vendor_plant_change_freq', 'pg_nominal_wacc', 'scrap_extra_charges','s.timestamp')\
     .agg(concat_ws(", ", collect_set('s.iopt_initiative_name')).alias('iopt_initiative_name'),
          concat_ws(", ", collect_set('s.iopt_initiative_pi_material_id')).alias('iopt_initiative_pi_material_id'),
          concat_ws(", ", collect_set('s.material_phase_out_phase_in')).alias('material_phase_out_phase_in'))
...
save_path = "/mnt/sbm_refined/abc_kinaxis_annual_runs_fact" if rtEnvironment =="Prod" else "/mnt/sbm_refined/abc_kinaxis_annual_runs_fact_dev"

#df_to_save.write.mode("overwrite").partitionBy("source_system_code").parquet(save_path)
agg_df.write.mode("overwrite").partitionBy("source_system_code").parquet(save_path)
